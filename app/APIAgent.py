...
try:
	...
	external_invoke_tool = safe_import(
		"app.tools_adapter", 
		"invoke_tool", 
		reason="外部工具调用功能将不可用"
	)
	external_retrieve_context = safe_import(
		"app.rag_adapter",
		"retrieve_context",
		reason="RAG检索功能将不可用"
	)
except ImportError:
	# 如果utils.imports也不可用，回退到原逻辑
	external_invoke_tool = None
	external_retrieve_context = None

# ===== 从 core_logic 导入共享工具函数（统一核心逻辑）=====
...
async def forward_to_model_stream(body: Dict[str, Any]):
	...
	async def _gen():
		...
		if _s1_ran:
			...
			# ── 鼓励多工具调用：在执行搜索后，如果RAG可用且问题可能需要知识库信息，也执行RAG检索 ──
			rag_enabled = os.getenv("GATEWAY_TOOL_RAG", "false").strip().lower() == "true"
			rag_query = last_user_msg[:80] if last_user_msg else ""
			
			# 判断是否需要RAG检索：如果问题包含知识库相关关键词或RAG已启用
			needs_rag = rag_enabled and bool(last_user_msg) and (
				# 如果已经有_kb_text（FC接管）则不需要重复执行
				not _kb_text and
				# 简单关键词检测：问题包含"知识库"、"文档"、"记忆"等关键词
				any(keyword in last_user_msg.lower() for keyword in ["知识库", "文档", "记忆", "kb", "knowledge", "document"])
			)
			
			if needs_rag and external_retrieve_context is not None:
				_t0 = time.monotonic()
				rag_hits = maybe_call_rag(rag_query, top_k=3)
				if rag_hits:
					rag_result_lines = [
						f"- {h.get('title') or h.get('source') or '片段'}: {h.get('body') or h.get('text') or str(h)}"
						for h in rag_hits
					]
					rag_result = "\n".join(rag_result_lines)
					# 将RAG结果合并到_kb_text中，供后续Step3/Step4使用
					_kb_text = (_kb_text + "\n\n" + rag_result).strip() if _kb_text else rag_result
					logging.warning("[step2/rag] executed elapsed=%.2fs result_len=%d query=%r",
						time.monotonic() - _t0, len(rag_result), rag_query[:60])
				else:
					logging.warning("[step2/rag] no hits found for query=%r", rag_query[:60])
...
async def rag_add(payload: Dict[str, Any]) -> JSONResponse:
	...
	try:
		from app.rag_adapter import add_document
